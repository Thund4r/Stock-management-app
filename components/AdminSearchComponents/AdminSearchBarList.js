...
export default function searchBarList({response_JSON, nameQuery}){
    ...
    const parseProductsJSON = async (response, nameQuery) => {
        if(response.ok){
            const arr_product_obj = await response.json()
            ...
            //http status is not correct need to deal with this.
        }
    }

    useEffect(() => {
        parseProductsJSON(response_JSON, nameQuery)
    }, [response_JSON])
